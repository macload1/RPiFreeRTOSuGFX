...
/*---------------------------------------------------------------------------*/
    ...
    #pragma \
    warning (disable:4711) /* function selected for automatic inline expansion */
#endif
/*---------------------------------------------------------------------------*/
...
#define PROPERTY_TYPE__CLEAN(type)   ((type) & \
                                      ~(PROPERTY_VALIDITY_BIT | \
                                        PROPERTY_NULLITY_BIT))
...
#define PROPERTY_TYPE__REAL__Q_LEFT__SHIFT      ( \
        PROPERTY_TYPE__REAL__Q_RIGHT__SHIFT + \
        PROPERTY_TYPE__REAL__Q_BITS_FOR_MASK)                                                                                                   /* 6u     */
#define PROPERTY_TYPE__REAL__Q_RIGHT__MASK      (((1u << \
                                                   PROPERTY_TYPE__REAL__Q_BITS) \
                                                  - 1u) << \
                                                 PROPERTY_TYPE__REAL__Q_RIGHT__SHIFT)
#define PROPERTY_TYPE__REAL__Q_LEFT__MASK       (((1u << \
                                                   PROPERTY_TYPE__REAL__Q_BITS) \
                                                  - 1u) << \
                                                 PROPERTY_TYPE__REAL__Q_LEFT__SHIFT)
#define PROPERTY_TYPE__REAL__Q_RIGHT__MAX_VALUE ((1u << \
                                                  PROPERTY_TYPE__REAL__Q_BITS) \
                                                 - 1u)                                        /* 31 */
#define PROPERTY_TYPE__REAL__Q_LEFT__MAX_VALUE  ((1u << \
                                                  PROPERTY_TYPE__REAL__Q_BITS) \
                                                 - 1u)                                        /* 31 */
#define PROPERTY_TYPE__REAL__MAX_BITS           (sizeof(uint32_t) * 8u)                         /* 32 */
#define PROPERTY_TYPE__REAL__ALL_1s             (~(uint32_t)0u)

#define PROPERTY_TYPE__QLEFT(type)             ((type & \
                                                 PROPERTY_TYPE__REAL__Q_LEFT__MASK) \
                                                >> \
                                                PROPERTY_TYPE__REAL__Q_LEFT__SHIFT)
#define PROPERTY_TYPE__QRIGHT(type)            ((type & \
                                                 PROPERTY_TYPE__REAL__Q_RIGHT__MASK) \
                                                >> \
                                                PROPERTY_TYPE__REAL__Q_RIGHT__SHIFT)

#define PROPERTY_TYPE__REAL__Q__INDICATES_UINT32(type) ((PROPERTY_TYPE__QLEFT ( \
                                                             type) == \
                                                         PROPERTY_TYPE__REAL__Q_LEFT__MAX_VALUE)  \
                                                        && ( \
                                                            PROPERTY_TYPE__QRIGHT( \
                                                                type) == \
                                                            PROPERTY_TYPE__REAL__Q_RIGHT__MAX_VALUE))

#define IS_SINGLE_VALUE(property_type)   (((property_type) & \
                                           PROPERTY_TYPE__MASK__OBJECT_OR_ARRAY_OR_VALUE) \
                                          == \
                                          PROPERTY_TYPE__SINGLE_VALUE__BASE)
#define IS_ARRAY(property_type)   (((property_type) & \
                                    PROPERTY_TYPE__MASK__OBJECT_OR_ARRAY_OR_VALUE) \
                                   == PROPERTY_TYPE__ARRAY__BASE)
#define IS_OBJECT(property_type)   ((property_type) & \
                                    PROPERTY_TYPE__OBJECT__BASE)

#define COMPLEX_OBJECT_LENGTH(property_type)   ((property_type) & \
                                                ~ \
                                                PROPERTY_TYPE__MASK__OBJECT_OR_ARRAY_OR_VALUE)
...
#define PROPERTY_TYPE__ARRAY__SIZE__MASK            (((1u << \
                                                       PROPERTY_TYPE__ARRAY__SIZE__BITS_FOR_MASK) \
                                                      - \
                                                      1u) << \
                                                     PROPERTY_TYPE__REAL__Q_RIGHT__SHIFT)                                                                /* 0x0FC0 */
#define PROPERTY_TYPE__ARRAY__SIZE__MIN             0
#define PROPERTY_TYPE__ARRAY__SIZE__MAX             ((1u << \
                                                      PROPERTY_TYPE__ARRAY__SIZE__BITS) \
                                                     - 1u)                                             /* 255 */

#if PROPERTY_TYPE__REAL__Q_BITS_FOR_MASK < PROPERTY_TYPE__REAL__Q_BITS
    #error Bad bits-allocation for Q
#endif
#if PROPERTY_TYPE__REAL__Q_BITS_FOR_MASK + \
    PROPERTY_TYPE__REAL__Q_RIGHT__SHIFT > 16u
    #error Field for right-Q spills over beyond uint16_t
#endif
#if PROPERTY_TYPE__REAL__Q_BITS_FOR_MASK + PROPERTY_TYPE__REAL__Q_LEFT__SHIFT > \
    16u
    #error Field for left-Q spills over beyond uint16_t
#endif
#if PROPERTY_TYPE__ARRAY__SIZE__BITS_FOR_MASK < \
    PROPERTY_TYPE__ARRAY__SIZE__BITS
    #error Error in bits-allocation for Array Size
#endif
#if PROPERTY_TYPE__ARRAY__SIZE__BITS_FOR_MASK + \
    PROPERTY_TYPE__REAL__Q_LEFT__SHIFT > 16u
    #error Field for array-size spills over beyond uint16_t
#endif
/*------------------------------------------------*/
/* __DEFAULTs For recovering from parsing errors. */
/*------------------------------------------------*/
#define PROPERTY_TYPE__REAL__Q_RIGHT__DEFAULT ((PROPERTY_TYPE__REAL__MAX_BITS / \
                                                2) << \
                                               PROPERTY_TYPE__REAL__Q_RIGHT__SHIFT)
#define PROPERTY_TYPE__REAL__Q_LEFT__DEFAULT  ((PROPERTY_TYPE__REAL__MAX_BITS - \
...
#define IS_TEMPLATE_PASS_PREPASS(parse_type)   (((parse_type) & 3) == \
                                                PARSE_PASS__TEMPLATE_PREPASS)
#define IS_TEMPLATE_PASS_FINAL(parse_type)   (((parse_type) & 3) == \
                                              PARSE_PASS__TEMPLATE_FINAL)
