...
#define MAX_INT32                           ((1uL << \
                                              (8 * sizeof(int32_t) - 1)) - 1uL)
#define MIN_INT32                           ((1uL << (8 * sizeof(int32_t) - 1)))

#define MAX_UINT16                          ((1uL << \
                                              (8 * sizeof(uint16_t))) - 1uL)

#define MAX_UINT8                           ((1uL << \
                                              (8 * sizeof(int8_t))) - 1uL)
/*---------------------------------------------------------------------------*/
#ifdef USE__PRETEND_TO_USE_PARAMETER
    #define PRETEND_TO_USE_PARAMETER(parameter)     PretendToUseParameter( \
        ( \
            const \
            void \
            *) \
...
/*---------------------------------------------------------------------------*/
    ...
    #ifdef _MSC_VER
        #pragma \
    warning(disable:4001)   /* nonstandard extension 'single line comment' was used */
    #endif
    ...
    #ifdef _MSC_VER
        #pragma \
    warning(default:4001)   /* nonstandard extension 'single line comment' was used */
    #endif
...
void UlToA(_I_ uint32_t val,
           __O char *   buf);                                               /* No native _ultoa in __TI_ARM__ */
    #elif defined ewarm
        #include <string.h>
        #define StrNiCmp     strncasecmp
void LToA(_I_ int32_t val,
          __O char *    buf);                                         /* No native _ltoa in ewarm */
void UlToA(_I_ uint32_t val,
           __O char *   buf);                                               /* No native _ultoa in ewarm */
        #elif defined __GCC__
                #include <strings.h>
                #define StrNiCmp     strncasecmp
void LToA(_I_ int32_t val,
