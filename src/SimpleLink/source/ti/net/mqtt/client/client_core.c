...
#define MQTTClientCore_isProtoVer31(clCtx)   (clCtx->flags & \
                                              MQTTCLIENTCORE_USE_PROTO_V31_FLAG)
#define MQTTClientCore_awaitsConnack(clCtx)  (clCtx->flags & \
                                              MQTTCLIENTCORE_CONNACK_AWAIT_FLAG)
#define MQTTClientCore_hasConnection(clCtx)  (clCtx->flags & \
                                              MQTTCLIENTCORE_NOW_CONNECTED_FLAG)
#define MQTTClientCore_awaitsKAPing(clCtx)   (clCtx->flags & \
                                              MQTTCLIENTCORE_KA_PINGER_RSP_FLAG)
#define MQTTClientCore_awaitsPingRsp(clCtx)  (clCtx->flags & \
                                              MQTTCLIENTCORE_USER_PING_RSP_FLAG)
#define MQTTClientCore_isClnSession(clCtx)   (clCtx->flags & \
                                              MQTTCLIENTCORE_CLEAN_SESSION_FLAG)
#define MQTTClientCore_recvTaskAvbl(clCtx)   (clCtx->flags & \
                                              MQTTCLIENTCORE_APP_RECV_TASK_FLAG)
#define MQTTClientCore_groupMember(clCtx)    (clCtx->flags & \
                                              MQTTCLIENTCORE_GROUP_CONTEXT_FLAG)
#define MQTTClientCore_needNetClose(clCtx)   (clCtx->flags & \
                                              MQTTCLIENTCORE_NETWORK_CLOSE_FLAG)
#define MQTTClientCore_cfgConnackTo(clCtx)   (clCtx->flags & \
                                              MQTTCLIENTCORE_DO_CONNACK_TO_FLAG)
#define MQTTClientCore_ctxCBsPtr(clCtx)     & (CLIENT(clCtx)->appCtxCBs)
#define CLIENT(clCtx)                        ((MQTTClientCore_ClientDesc_t *) \
                                              clCtx)
...
int32_t (*debugPrintf)(const char *fmt,
                       ...) = NULL;
...
static uint8_t MQTTClientCore_mqtt310[] =
{ 0x00, 0x06, 'M', 'Q', 'I', 's', 'd', 'p', 0x03 };
static uint8_t MQTTClientCore_mqtt311[] =
...
static MQTTClientCore_ModuleStates MQTTClientCore_clLibState =
    ModuleState_WaitInit;
...
//!
//*****************************************************************************
static bool txPartSetup(MQTTClientCore_txPartPkt_t *txPart,
...
//!
//*****************************************************************************
static void txPartAddup(MQTTClientCore_txPartPkt_t *txPart,
                        uint32_t offset)
{
...
static int32_t loopbTrigger(void)
{
    ...
    return ((-1 != MQTTClientCore_loopbNet) ?
            MQTTClientCore_netOps->sendTo(MQTTClientCore_loopbNet,
                                          MQTTClientCore_loopData,
                                          MQTTCLIENTCORE_LOOP_DLEN,
                                          MQTTClientCore_loopbPortid, ip_addr,
                                          4) : MQTT_PACKET_ERR_LIBQUIT);
}
...
//!
//*****************************************************************************
static void doNetCloseRx(MQTT_ClientCtx_t *clCtx,
                         int32_t cause)
{
...
//!
//*****************************************************************************
static void doNetCloseTx(MQTT_ClientCtx_t *clCtx,
                         char *cause)
{
...
//!
//*****************************************************************************
static bool qos2PubRxLogup(MQTT_ClientCtx_t *clCtx,
                           uint16_t msgID)
{
...
//!
//*****************************************************************************
static bool ack2MsgIdLogup(MQTT_ClientCtx_t *clCtx,
                           uint16_t msgID)
{
...
//!
//*****************************************************************************
static bool qos2PubRxUnlog(MQTT_ClientCtx_t *clCtx,
                           uint16_t msgID)
{
...
//!
//*****************************************************************************
static bool ack2MsgIdUnlog(MQTT_ClientCtx_t *clCtx,
                           uint16_t msgID)
{
    MQTTClientCore_ClientDesc_t *client = CLIENT(clCtx);
...
//!
//*****************************************************************************
static bool qos2PubRxIsDone(MQTT_ClientCtx_t *clCtx,
                            uint16_t msgID)
{
...
static bool awaitsPkts(MQTT_ClientCtx_t *clCtx)
{
    ...
    return (client->qosAck1WaitList.head ||
            MQTT_qos2PubCqCount(&client->rxQos2CircularQueue) ||
...
{
    return ((MQTTClientCore_hasConnection(clCtx) &&
             !MQTTClientCore_needNetClose(clCtx))
            ? true : false);
}
...
//!
//*****************************************************************************
static inline int32_t netSend(int32_t net,
                              const uint8_t *buf,
                              uint32_t len,
                              void *ctx)
{
...
static int32_t clCtxPartSend(MQTT_ClientCtx_t *clCtx)
{
    ...
    MQTTCLIENTCORE_USR_INFO("C: %s 0x%02x to net %d, %s (%d Bytes) [@ %u]\n\r",
                            ofs ? "PartN" : "FH-B1", B1, clCtx->net,
                            (rv > 0) ? "Sent" : "Fail", rv,
                            MQTTClientCore_netOps->time());
...
//!
//*****************************************************************************
static int32_t clCtxSeg1Send(MQTT_ClientCtx_t *clCtx,
                             const uint8_t *buf,
                             uint32_t len,
                             bool is_conn_msg,
                             MQTT_Packet_t *mqpTx)
...
//!
//*****************************************************************************
static int32_t wrConnectPayload(MQTT_ClientCtx_t *clCtx,
                                uint8_t *buf,
                                uint32_t fsz,
                                uint8_t *conn_opts)
{
    ...
    uint8_t utf8_sel[] =
    { 0x00, MQTT_CONN_MSG_FLAG_WILL_CONFIG, 0x00, MQTT_CONN_MSG_FLAG_USER_NAME,
      MQTT_CONN_MSG_FLAG_PASS_WORD };
    MQTTClientCore_ClientDesc_t *client = CLIENT(clCtx);
    uint8_t *ref = buf;
    int32_t i = 0;
...
static inline uint16_t getConnectVhLen(MQTT_ClientCtx_t *clCtx)
{
    return ((MQTTClientCore_isProtoVer31(clCtx) ?
             sizeof(MQTTClientCore_mqtt310) : sizeof(MQTTClientCore_mqtt311)) +
            3);
}
...
//!
//*****************************************************************************
static int32_t wrConnectVh(MQTT_ClientCtx_t *clCtx,
                           uint8_t *buf,
                           uint16_t kaSecs,
                           uint8_t conn_opts)
{
    ...
    {
        buf +=
            MQTT_bufWrNbytes(buf, MQTTClientCore_mqtt310,
                             sizeof(MQTTClientCore_mqtt310));
    }
    else
    {
        buf +=
            MQTT_bufWrNbytes(buf, MQTTClientCore_mqtt311,
                             sizeof(MQTTClientCore_mqtt311));
    }
    *buf++ = conn_opts;
...
static int32_t netConnect(MQTT_ClientCtx_t *clCtx)
{
    ...
    clCtx->net = MQTTClientCore_netOps->open(
        client->nwconnOpts | MQTT_DEV_NETCONN_OPT_TCP,
        client->serverAddr,
        client->portNumber, &client->nwSecurity);
...
//!
//*****************************************************************************
static int32_t clCtxConnStateTryLocked(MQTT_ClientCtx_t *clCtx,
                                       const uint8_t *buf,
                                       uint32_t len,
                                       uint16_t kaSecs,
                                       bool cleanSession,
                                       MQTT_Packet_t *mqpTx)
{
    ...
    if(rv == 0)
    {
        ...
        if(false ==
           ((ModuleState_InitDone != MQTTClientCore_clLibState) ||
            (MQTTClientCore_awaitsConnack(clCtx))))
        {
        ...
        if(rv >= 0)
        {
            ...
            clCtx->flags |= MQTTCLIENTCORE_DO_CONNACK_TO_FLAG |
                            MQTTCLIENTCORE_CONNACK_AWAIT_FLAG;
            clCtx->flags |=
                cleanSession ? MQTTCLIENTCORE_CLEAN_SESSION_FLAG : 0;
...
//!
//*****************************************************************************
static int32_t connectMsgSend(MQTT_ClientCtx_t *clCtx,
                              bool cleanSession,
                              uint16_t kaSecs)
{
    MQTT_Packet_t *mqp = MQTTClientCore_allocSend(MQTT_CONNECT);
    uint8_t *buf, *ref, conn_opts =
        cleanSession ? MQTT_CONN_MSG_FLAG_CLEAN_START : 0;
    ...
    if(fsz >= vhl)
    {
        ...
        if(rv >= 0)
        {
            ...
            rv =
                clCtxConnStateTryLocked(clCtx, ref, buf - ref, kaSecs,
                                        cleanSession,
                                        mqp);
            if(rv > 0)
            {
...
//!
//*****************************************************************************
static int32_t _msgDispatch(MQTT_ClientCtx_t *clCtx,
                            MQTT_Packet_t *mqp,
                            MQTT_QOS qos,
                            bool retain)
{
    ...
    rv = clCtxSeg1Send(clCtx, MQTT_PACKET_FHEADER_BUF(
                           mqp), MQTT_PACKET_CONTENT_LEN(mqp), false, mqp);
...
//!
//*****************************************************************************
static int32_t msgDispatchNoFree(MQTT_ClientCtx_t *clCtx,
                                 MQTT_Packet_t *mqp,
                                 MQTT_QOS qos,
                                 bool retain)
{
...
//!
//*****************************************************************************
static int32_t bufUTF8WrTry(uint8_t *buf,
                            uint32_t fsz,
...
//!
//*****************************************************************************
static int32_t utf8ArraySend(MQTT_ClientCtx_t *clCtx,
                             const MQTT_UTF8StrQOS_t *subsc_vec,
                             const MQTT_UTF8String_t *unsub_vec,
                             uint32_t n_elem)
{
    MQTT_Packet_t *mqp;
    uint8_t *ref, *buf, *end;
    ...
    for(i = 0; i < n_elem; i++)
    {
        ...
        rv = bufUTF8WrTry(buf, end - buf, topic,
                          subsc_vec ? (uint8_t) subsc_vec[i].qosreq : QFL_VALUE);
        if(rv < 0)
        {
            mqpFreeLocked(mqp);
            return(rv);
...
//!
//*****************************************************************************
static int32_t vhMsgSend(MQTT_ClientCtx_t *clCtx,
                         uint8_t msgType,
                         MQTT_QOS qos,
                         bool hasVH,
                         uint16_t vhData)
...
//!
//*****************************************************************************
static int32_t pingReqSend(MQTT_ClientCtx_t *clCtx,
                           uint32_t rsp_flag)
{
    int32_t rv = 0;
    uint8_t buf[2];
    ...
    buf[0] =
        MQTT_MAKE_FH_BYTE1(MQTT_PINGREQ,
                           MQTT_MAKE_FH_FLAGS(false, MQTT_QOS0, false));
    buf[1] = 0;
...
static bool ack1WlMqpDispatch(MQTT_ClientCtx_t *clCtx)
{
    ...
    for(mqp = wlist->head; mqp && (true == rv); mqp = mqp->next)
    {
        ...
        if(clCtxSeg1Send(clCtx, buf, MQTT_PACKET_CONTENT_LEN(mqp),false,
                         mqp) <= 0)
        {
...
//!
//*****************************************************************************
static bool ack1WlRmfree(MQTT_AckWlist_t *wl,
                         uint16_t msgID)
{
...
//!
//*****************************************************************************
static bool _procPubRecRx(MQTT_ClientCtx_t *clCtx,
                          uint16_t msgID)
{
...
//!
//*****************************************************************************
static bool _procPubRelRx(MQTT_ClientCtx_t *clCtx,
                          uint16_t msgID)
{
...
//!
//*****************************************************************************
static bool _procAckMsgRx(MQTT_ClientCtx_t *clCtx,
                          MQTT_Packet_t *mqpRaw)
{
    MQTTClientCore_CtxCBs_t *ctxCBs = MQTTClientCore_ctxCBsPtr(clCtx);
    MQTTClientCore_ClientDesc_t *client = CLIENT(clCtx);
    ...
    if(ctxCBs->ackNotify)
    {
        ctxCBs->ackNotify(client->app, mqpRaw->msgType, msgID,
                          len ? MQTT_PACKET_PAYLOAD_BUF(
                              mqpRaw) : NULL, len);
    }
    return(true);
}
...
//!
//*****************************************************************************
static bool procAckMsgRx(MQTT_ClientCtx_t *clCtx,
                         MQTT_Packet_t *mqpRaw)
{
...
//!
//*****************************************************************************
static bool procPubMsgRx(MQTT_ClientCtx_t *clCtx,
                         MQTT_Packet_t *mqpRaw)
{
    MQTTClientCore_CtxCBs_t *ctxCBs = MQTTClientCore_ctxCBsPtr(clCtx);
    ...
    if(ctxCBs->publishRx)
    {
        ...
        if(ctxCBs->publishRx(CLIENT(clCtx)->app, MQTT_FH_BYTE1_IS_DUP(B), qos,
                             MQTT_FH_BYTE1_IS_RETAIN(B), mqpRaw))
        {
...
//!
//*****************************************************************************
static bool procConnackRx(MQTT_ClientCtx_t *clCtx,
                          MQTT_Packet_t *mqpRaw)
{
    MQTTClientCore_CtxCBs_t *ctxCBs = MQTTClientCore_ctxCBsPtr(clCtx);
    ...
    clCtx->flags &=
        ~(MQTTCLIENTCORE_DO_CONNACK_TO_FLAG | MQTTCLIENTCORE_CONNACK_AWAIT_FLAG);
    ...
    if(!(MQTTClientCore_vhbConnackRC(buf)))
    {
        ...
        if(MQTTClientCore_isClnSession(clCtx) ||
           (0 == MQTTClientCore_vhbConnackSP(buf)))
        {
            sessionDelete(clCtx);
        }
        else
...
//!
//*****************************************************************************
static bool procPingrspRx(MQTT_ClientCtx_t *clCtx,
                          MQTT_Packet_t *mqpRaw)
{
    MQTTClientCore_CtxCBs_t *ctxCBs = MQTTClientCore_ctxCBsPtr(clCtx);
...
//!
//*****************************************************************************
static bool connSentStateRx(MQTT_ClientCtx_t *clCtx,
                            MQTT_Packet_t *mqpRaw)
{
    bool rv = false;
...
//!
//*****************************************************************************
static bool connectedStateRx(MQTT_ClientCtx_t *clCtx,
                             MQTT_Packet_t *mqpRaw)
{
    bool rv = false;
...
//!
//*****************************************************************************
static bool processRecv(MQTT_ClientCtx_t *clCtx,
                        MQTT_Packet_t *mqpRaw)
{
    bool rv;

    MQTTCLIENTCORE_USR_INFO(
        "C: Rcvd msg Fix-Hdr (Byte1) 0x%02x from net %d [@ %u]\n\r",
        mqpRaw->fhByte1,
        clCtx->net, MQTTClientCore_netOps->time());
     ...
     */
    rv =
        MQTTClientCore_awaitsConnack(clCtx) ? connSentStateRx(clCtx,
                                                              mqpRaw) :
        connectedStateRx(
            clCtx, mqpRaw);
...
//!
//*****************************************************************************
static int32_t netRecv(int32_t net,
                       MQTT_Packet_t *mqp,
                       uint32_t waitSecs,
                       void *ctx)
{
    bool timedOut = false;
    int32_t rv =
        MQTT_PacketRecv(net, MQTTClientCore_netOps, mqp, waitSecs, &timedOut,
                        ctx);
    ...
    if(rv <= 0)
    {
        ...
        {
            MQTTCLIENTCORE_USR_INFO(
                "C: Net %d, Raw Error %d, Time Out: %c [%u]\n\r", net, rv,
                timedOut ? 'Y' : 'N', MQTTClientCore_netOps->time());
        }
    }
...
//!
//*****************************************************************************
static int32_t singleCtxKaSequence(MQTT_ClientCtx_t *clCtx,
                                   uint32_t waitSecs)
{
...
//!
//*****************************************************************************
static uint32_t singleCtxAdjWaitSecsGet(MQTT_ClientCtx_t *clCtx,
                                        uint32_t waitSecs)
{
    ...
    return((MQTT_KA_TIMEOUT_NONE != clCtx->timeout) ? MIN(secs,
                                                          waitSecs) : waitSecs);
}
...
//!
//*****************************************************************************
static int32_t singleCtxRxPrep(MQTT_ClientCtx_t *clCtx,
                               uint32_t *secs2wait)
{
    int32_t rv;
...
//!
//*****************************************************************************
static int32_t singleCtxRxPrepLocked(MQTT_ClientCtx_t *clCtx,
                                     uint32_t *secs2wait)
{
    int32_t rv;
...
//!
//*****************************************************************************
static int32_t procCtxDataRecv(MQTT_ClientCtx_t *clCtx,
                               MQTT_Packet_t *mqp,
                               uint32_t waitSecs,
                               void **app)
{
    ...
    *app = clCtx->usr;
     ...
     */
    if((1 == MQTTClient_closeContext) ||
       ((rv < 0) && (rv != MQTT_PACKET_ERR_TIMEOUT)) ||
...
//!
//*****************************************************************************
static int32_t mqpSetupProcCtxDataRecv(MQTT_ClientCtx_t *clCtx,
                                       MQTT_Packet_t *mqp,
                                       uint32_t waitSecs,
                                       void **app)
{
...
//!
//*****************************************************************************
static int32_t clCtxRecv(MQTT_ClientCtx_t *clCtx,
                         MQTT_Packet_t *mqp,
                         uint32_t waitSecs)
{
    void *app = NULL;
...
{
    return (MQTTClientCore_usedCtxs ? singleCtxAdjWaitSecsGet(
                MQTTClientCore_usedCtxs, waitSecs) : waitSecs);
}
...
//!
//*****************************************************************************
static void recvHvecLoad(int32_t *hvec_recv,
                         uint32_t size,
                         MQTT_ClientCtx_t *list)
{
    int32_t i = 0;
...
//!
//*****************************************************************************
static int32_t groupCtxsRxPrep(uint32_t waitSecs,
                               void **app)
{
    ...
    recvHvecLoad(&MQTTClientCore_recvHvec[1], MQTTCLIENTCORE_MAX_NWCONN + 1,
                 MQTTClientCore_usedCtxs);

    waitSecs = groupCtxsAdjWaitSecsGet(waitSecs);

    n_hnds =
        MQTTClientCore_netOps->ioMon(MQTTClientCore_recvHvec,
                                     &MQTTClientCore_sendHvec,
                                     &MQTTClientCore_rsvdHvec,
                                     waitSecs);
    if(0 == n_hnds)
    {
...
static int32_t procLoopbackRecv(int32_t net)
{
    ...
    int32_t rv =
        MQTTClientCore_netOps->recvFrom(net, buf, MQTTCLIENTCORE_LOOP_DLEN,
                                        NULL,
                                        NULL,
                                        0);
...
//!
//*****************************************************************************
static int32_t procNetDataRecv(int32_t net,
                               MQTT_Packet_t *mqp,
                               void **app)
{
...
//!
//*****************************************************************************
static int32_t clRecv(MQTT_Packet_t *mqp,
                      uint32_t waitSecs,
                      void **app)
{
    int32_t rv = MQTT_PACKET_ERR_NETWORK;
...
static int32_t grpNetSetupCreate()
{
    ...
    {
        MQTTClientCore_loopbNet = MQTTClientCore_netOps->open(
            MQTT_DEV_NETCONN_OPT_UDP, NULL, MQTTClientCore_loopbPortid, NULL);
        if(-1 == MQTTClientCore_loopbNet)
        {
            return(MQTT_PACKET_ERR_LIBQUIT);
        }
                       ...
                       void *app)
{
    ...
    {
        MQTT_bufWrNbytes((uint8_t*) &client->nwSecurity,
                         (uint8_t*) ctxCfg->nwSecurity,
                         sizeof(MQTT_SecureConn_t));
    }
    if(NULL != ctxCBs)
    {
        // set callback flag
...
int32_t MQTTClientCore_sendConnectMsg(void *ctx,
                                      ...
                                      uint16_t kaSecs)
{
    return (ctx ? connectMsgSend((MQTT_ClientCtx_t*)(ctx), cleanSession,
                                 kaSecs) : -1);
}
                                  ...
                                  bool retain)
{
    ...
    if((0 >
        MQTT_packetPubAppendTopic(mqp, topic,
                                  qos ? assignNewMsgID() : 0)) ||
...
int32_t MQTTClientCore_sendPingreq(void *ctx)
{
    ...
    MQTTClientCore_mutexLockin();
    rv = pingReqSend((MQTT_ClientCtx_t*)(ctx),
                     MQTTCLIENTCORE_USER_PING_RSP_FLAG);
    MQTTClientCore_mutexUnlock();
...
int32_t MQTTClientCore_sendDisconn(void *ctx)
{
    ...
    buf[0] =
        MQTT_MAKE_FH_BYTE1(MQTT_DISCONNECT,
                           MQTT_MAKE_FH_FLAGS(false, MQTT_QOS0, false));
    buf[1] = 0;
...
int32_t MQTTClientCore_run(uint32_t waitSecs)
{
    ...
    while((rv > 0) ||
          ...
          ((rv != MQTT_PACKET_ERR_LIBQUIT) &&
           (rv != MQTT_PACKET_ERR_TIMEOUT) && (rv != MQTT_PACKET_ERR_NETWORK)));
...
{
    if(net && net->open && net->send && net->recv && net->sendTo &&
       net->recvFrom && net->close && net->ioMon && net->time)
    {
                                 ...
                                 void **ctx)
{
    ...
    if((NULL == ctxCfg) || (NULL == ctxCfg->serverAddr) ||
       (0 == ctxCfg->portNumber))
    {
        return(-1);
    }
    if((ctxCfg->configOpts & MQTTCLIENTCORE_CFG_MK_GROUP_CTX) &&
       (MQTTClientCore_grpHasCBFn ^ (!!ctxCBs)))
    {
        return(-1);
    }
