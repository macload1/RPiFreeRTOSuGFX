...
#define MQTT_PACKET_VHEADER_BUF(mqp)  (MQTT_PACKET_FHEADER_BUF(mqp) + \
                                       mqp->fhLen)
#define MQTT_PACKET_PAYLOAD_BUF(mqp)  (MQTT_PACKET_VHEADER_BUF(mqp) + \
                                       mqp->vhLen)
...
#define MQTT_PACKET_PUB_PAY_BUF(mqp) (mqp->plLen ? MQTT_PACKET_PAYLOAD_BUF(mqp) \
                                      : NULL)
...
typedef struct _MQTT_DeviceNetServices_t_
{
     ...
     */
    int32_t (*open)(uint32_t nwconnOpts,
     ...
     */
    int32_t (*listen)(uint32_t nwconnOpts,
                      uint16_t portNumber,
                      const MQTT_SecureConn_t *nwSecurity);
    ...
    /** Monitor activity on communication handles.
        ...
        error.
     */
    int32_t (*ioMon)(int32_t *recvCvec,
                     int32_t *sendCvec,
                     int32_t *rsvdCvec,
                     uint32_t waitSecs);
...
static inline uint32_t MQTT_bufWrNbytes(uint8_t *dst,
                                        const uint8_t *src,
                                        uint32_t n)
{
    uint32_t c = n;
...
static inline uint32_t MQTT_bufSet(uint8_t *dst,
                                   uint8_t val,
                                   uint32_t n)
{
    uint32_t c = n;
...
static inline uint32_t MQTT_bufWrNbo2B(uint8_t *buf,
                                       uint16_t val)
{
...
static inline uint32_t MQTT_bufRdNbo2B(const uint8_t *buf,
                                       uint16_t *val)
{
...
static
inline void MQTT_packetBufferAttach(MQTT_Packet_t *mqp,
                                    uint8_t *buffer,
                                    uint32_t length,
                                    uint8_t offset)
{
 ...
 */
int32_t
MQTT_packetPubAppendTopic(MQTT_Packet_t *mqp,
