...
#define MQTTClient_str2UTFConv(utf_s, str) {utf_s.buffer = (char *)str; \
                                            utf_s.length = strlen(str); }
...
#define MQTTClient_rxTxSemWait()     sem_wait( \
        &MQTTClient_Lib_CB.MQTTClient_syncRxTx)
#define MQTTClient_rxTxSemPost()     sem_post( \
        &MQTTClient_Lib_CB.MQTTClient_syncRxTx)

#define MQTTClient_mutexLock()   pthread_mutex_lock( \
        &MQTTClient_Lib_CB.MQTTClient_mutex);                                             // forever
#define MQTTClient_mutexUnlock() pthread_mutex_unlock( \
        &MQTTClient_Lib_CB.MQTTClient_mutex);
...
typedef struct _SlMQTTClient_LibCfg_t_
{
    ...
    int32_t (*dbgPrint)(const char *pcFormat,
...
static SlMQTTClient_Ctx_t SlMQTTClient_ctx[
    MQTTCLIENT_MAX_SIMULTANEOUS_SERVER_CONN];
...
//
//*****************************************************************************
static inline void SetCondAwaitedAckLocked(SlMQTTClient_Ctx_t *clientCtx,
                                           char cond,
                                           char val)
{
...
int32_t _sl_ExtLib_ServerAckLockedWait(SlMQTTClient_Ctx_t *clientCtx)
{
    ...
    if(true == clientCtx->blockingSend)
    {
        ...
        rc =
            (MQTT_DISCONNECT ==
             clientCtx->awaitedAck) ? MQTT_PACKET_ERR_NOTCONN : 0;
    }
...
//
//*****************************************************************************
bool _sl_ExtLib_AwaitedAckSet(SlMQTTClient_Ctx_t *clientCtx,
                              uint8_t newAwaited)
{
    bool rc = false;
...
//
//*****************************************************************************
bool _sl_ExtLib_AwaitedAckLockedSet(SlMQTTClient_Ctx_t *clientCtx,
                                    uint8_t newAwaited)
{
    bool rc;
...
//
//*****************************************************************************
static void processNotifyAckCB(void *app,
                               uint8_t msgType,
                               uint16_t msgID,
                               uint8_t *buf,
                               uint32_t len)
{
    ...
    case MQTT_CONNACK:
        ...
        clientCtx->appCBs(MQTT_CLIENT_OPERATION_CB_EVENT, (void*)&eventCB,
                          sizeof(eventCB), (void*)&(clientCtx->connAck),
                          sizeof(clientCtx->connAck));
    ...
    default:
        ...
        else
        {
            ...
            clientCtx->appCBs(MQTT_CLIENT_OPERATION_CB_EVENT, (void*)&eventCB,
                              sizeof(eventCB), (void*)MQTTCLIENT_ACK, strlen(
                                  MQTTCLIENT_ACK));
        }
        break;
    }
...
//
//*****************************************************************************
static bool processPublishRxCB(void *app,
                               bool dup,
                               MQTT_QOS qos,
                               bool retain,
                               MQTT_Packet_t *mqp)
{
    ...
    if(MQTT_DISCONNECT != clientCtx->awaitedAck)
    {
        ...
        clientCtx->appCBs(MQTT_CLIENT_RECV_CB_EVENT, &metaData, sizeof(metaData),
                          MQTT_PACKET_PUB_PAY_BUF(
                              mqp), MQTT_PACKET_PUB_PAY_LEN(mqp));
    }
...
//
//*****************************************************************************
static void processDisconnCB(void *app,
                             int32_t cause)
{
    SlMQTTClient_Ctx_t *clientCtx = (SlMQTTClient_Ctx_t *) app;
...
int32_t MqttClientCtxCreate(_const MQTTClient_Attrib_t *ctxCfg)
{
    ...
    {
        libCtxCfg.configOpts |=
            (MQTTCLIENTCORE_CFG_APP_HAS_RTSK | MQTTCLIENTCORE_CFG_MK_GROUP_CTX);
    }
    else
    {
    ...
    if((libCtxCfg.nwconnOpts & MQTT_DEV_NETCONN_OPT_SEC) != 0)
    {
        ...
        if((libCtxCfg.nwconnOpts & MQTT_DEV_NETCONN_OPT_UDP) == 0)
        {
               ...
               ctxCfg->connParams->nFiles)
            {
                ...
                /* initialize secure socket parameters */
                libCtxCfg.nwSecurity->cipher =
                    (void*) &(ctxCfg->connParams->cipher);
                libCtxCfg.nwSecurity->method =
                    (void*) &(ctxCfg->connParams->method);
                libCtxCfg.nwSecurity->nFile = ctxCfg->connParams->nFiles;
                libCtxCfg.nwSecurity->files =
                    (char**) ctxCfg->connParams->secureFiles;
            }
            else
            {
    ...
    retval = MQTTClientCore_createCtx(&libCtxCfg, &libCliCBs, clientCtxPtr,
                                      &clientCtxPtr->cliHndl);
...
{
    SlMQTTClient_Ctx_t *clientCtx =
        (SlMQTTClient_Ctx_t *) MQTTClient_clientContext;
    int32_t retval;
...
//
//*****************************************************************************
int32_t MqttClientSub(MQTTClient_SubscribeParams_t* pValue,
                      int32_t count)
{
...
//
//*****************************************************************************
int32_t MqttClientUnsub(MQTTClient_UnsubscribeParams_t* pValue,
                        int32_t count)
{
    int32_t ret = MQTT_PACKET_ERR_FNPARAM, i;
...
int32_t MqttClientConfig(_const SlMQTTClient_LibCfg_t *cfg)
{
    ...
    retval =
        pthread_mutex_init(&MQTTClient_Lib_CB.MQTTClient_libLock,
    ...
    /* setup the mutex operations */
    retval =
        pthread_mutex_init(&MQTTClient_Lib_CB.MQTTClient_mutex,
                           (const pthread_mutexattr_t *)NULL);
    if(retval < 0)
    {
...
//
//*****************************************************************************
NetAppHandle_t MQTTClient_create(NetAppCallBack_t defaultCallback,
                                 MQTTClient_Attrib_t *attrib)
{
    ...
    MQTTClientCore_registerBuffers(MQTTCLIENT_MAX_MQP, MQTTClient_packet,
                                   MQTTCLIENT_BUF_LEN,
                                   &MQTTClient_buffer[0][0]);
    ...
    do
    {
        if(false == MQTTClient_multiSrvConn)
        {
            ...
            while(1)
            {
                ...
                retval = MQTTClientCore_ctxRun(SlMQTTClient_ctx[0].cliHndl,
                                               MQTTClient_waitSecs);

                if(((retval < 0) &&
                    (retval != MQTT_PACKET_ERR_TIMEOUT)) ||
                   (1 == MQTTClient_closeContext))
                {
...
int16_t MQTTClient_connect(NetAppHandle_t handle)
{
    ...
    ret = MQTTClientCore_registerCtxInfo(clientCtx->cliHndl, clientid, usrname,
                                         usrpasswd);
    if(ret < 0)
    {
        clientCtx->connAck = 0;
    ...
    if(NULL != clientCtx->mqttWill.willTopic)
    {
        ...
        ret = MQTTClientCore_registerWillInfo(
            clientCtx->cliHndl, &will_topic, &willMsg,
            MQTTClient_qos[clientCtx->mqttWill.willQos],
            clientCtx->mqttWill.retain);
        if(ret < 0)
        {
            clientCtx->connAck = 0;
            MQTTClient_mutexUnlock();
    ...
    ret =
        MQTTClientCore_sendConnectMsg(clientCtx->cliHndl,
                                      MQTTClient_connectCfg.clean,
                                      MQTTClient_connectCfg.keepAliveTimeout);
...
int16_t MQTTClient_disconnect(NetAppHandle_t handle)
{
    ...
    if(_sl_ExtLib_AwaitedAckLockedSet(clientCtx,
                                      MQTTCLIENT_DISCONNECT_CB) == true)
    {
...
//
//*****************************************************************************
uint16_t MqttClient_send(NetAppHandle_t hdl,
                         void* pMeta,
                         uint16_t MetaLen,
                         void* pData,
                         uint16_t DataLen,
                         uint32_t flags)
{
    ...
    /*publish the message*/
    ret =
        MQTTClientCore_sendPubMsg(clientCtx->cliHndl, &topicUTF8, pData,
                                  DataLen,
                                  MQTTClient_qos[qosLevel],
                                  retain);
    if(ret < 0)
    {
...
//
//*****************************************************************************
int16_t MQTTClient_publish(NetAppHandle_t handle,
                           char *topic,
                           uint16_t topicLen,
                           char *msg,
                           uint16_t msgLen,
                           uint32_t flags)
{
    return (MqttClient_send(handle, (void*)topic,topicLen, (void*) msg,  msgLen,
                            flags));
}
...
//
//*****************************************************************************
int16_t MQTTClient_subscribe(NetAppHandle_t handle,
                             MQTTClient_SubscribeParams_t *value,
                             uint8_t numberOfTopics)
{
    return (MqttClientSub((MQTTClient_SubscribeParams_t *)value,
                          (int32_t)numberOfTopics));
}
...
//
//*****************************************************************************
int16_t MQTTClient_unsubscribe(NetAppHandle_t handle,
                               MQTTClient_UnsubscribeParams_t *value,
                               uint8_t numberOfTopics)
{
    return (MqttClientUnsub((MQTTClient_UnsubscribeParams_t*)value,
                            numberOfTopics));
}
...
//
//*****************************************************************************
int16_t MQTTClient_get(NetAppHandle_t handle,
                       uint16_t option,
                       void *value,
...
//
//*****************************************************************************
int16_t MQTTClient_set(NetAppHandle_t handle,
                       uint16_t option,
                       void *value,
                       uint16_t valueLength)
{
