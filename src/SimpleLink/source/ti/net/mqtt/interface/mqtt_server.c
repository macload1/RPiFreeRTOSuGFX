...
uint16_t SlMQTTServerConnectCB(_const MQTT_UTF8String_t *clientId,
                               ...
                               _const MQTT_UTF8String_t *password,
                               void **appUsr);

void SlMQTTServerPublishCB(_const MQTT_UTF8String_t *topic,
                           _const uint8_t *payload,
                           uint32_t payLen,
                           bool dup,
                           uint8_t qos,
                           bool retain);

void SlMQTTServerDisconnCB(_const void *appUsr,
                           bool due2err);
                               ...
                               void **appUsr)
{
    ...
    if(strlen(MQTTServer_LocalAuthenticationData.userName) > 0)
    {
        ...
        if((username != NULL) &&
           strcmp(MQTTServer_LocalAuthenticationData.userName,
                  meta.userName) == 0)
        {
            /* Comparing the server password with the password that the   */
            /* client sent in the connection request                      */
            if((password != NULL) &&
               strcmp(MQTTServer_LocalAuthenticationData.password,
                      meta.password) == 0)
            {
                MQTTServer_cbs(MQTT_SERVER_CONNECT_CB_EVENT, &meta, sizeof(meta),
                               NULL,
                               0);
                return(0);
            }
    ...
    else
    {
        ...
        MQTTServer_cbs(MQTT_SERVER_CONNECT_CB_EVENT, &meta, sizeof(meta), NULL,
                       0);
        return(0);
    }
    ...
    /* not match                                                              */
    MQTTServer_cbs(MQTT_SERVER_CONNECT_CB_EVENT, &meta, sizeof(meta),
                   (void *)MQTT_CONNACK_RC_BAD_USRPWD,
                   sizeof(int));
                           ...
                           bool retain)
{
    ...
    meta.dup = dup;
    meta.retain = retain;
    MQTTServer_cbs(MQTT_SERVER_RECV_CB_EVENT, &meta,
                   sizeof(MQTTServer_RecvMetaDataCB_t), (char*)payload, payLen);
}
                           ...
                           bool due2err)
{
    ...
    MQTTServer_cbs(MQTT_SERVER_DISCONNECT_CB_EVENT, &meta,
                   sizeof(MQTTServer_DisconnectMetaDataCB_t), NULL, 0);
}
...
//
//*****************************************************************************
uint16_t MQTTServerSend(NetAppHandle_t hdl,
                        void* meta,
                        uint16_t metaLen,
                        void* data,
                        uint16_t dataLen,
                        uint32_t flags)
{
    ...
    return (MQTTServerCore_pubSend(&topicUTF8, data, dataLen,
                                   (MQTT_QOS) qos_level, retain));
}
...
int32_t MQTTServerTopicEnroll(_const char *topic)
{
    ...
    return (MQTTServerCore_topicEnroll((void*) (MQTTServer_appHndl.pluginHndl),
                                       &topicUTF8, MQTT_QOS2));
}
...
int32_t MQTTServerTopicDisenroll(_const char *topic)
{
    ...
    return (MQTTServerCore_topicDisenroll((void*) &(MQTTServer_appHndl.
                                                    pluginHndl), &topicUTF8));
}
...
//
//*****************************************************************************
NetAppHandle_t MQTTServer_create(NetAppCallBack_t defaultCallback,
                                 MQTTServer_Attrib_t *attrib)
{
    ...
    ret = pthread_mutex_init(&MQTTServer_mutex,
    ...
    MQTTServer_appHndl.pluginHndl = MQTTServerCore_appRegister(
        &MQTTServer_appCBs, "temp");
...
//
//*****************************************************************************
int16_t MQTTServer_subscribe(NetAppHandle_t handle,
                             MQTTServer_SubscribeParams_t *value,
...
//
//*****************************************************************************
int16_t MQTTServer_unsubscribe(NetAppHandle_t handle,
                               MQTTServer_UnSubscribeParams_t *value,
                               uint8_t numberOfTopics)
{
    int i;
...
//
//*****************************************************************************
int16_t MQTTServer_publish(NetAppHandle_t handle,
                           char *topic,
                           uint16_t topicLen,
                           char *msg,
                           uint16_t msgLen,
...
//
//*****************************************************************************
int16_t MQTTServer_set(NetAppHandle_t handle,
                       uint16_t option,
                       void *value,
