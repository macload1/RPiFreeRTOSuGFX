...
#define MQTTClientMgmt_isClUsrFree(usr) ((0 == usr->flags) && \
                                         ('\0' == usr->clientID[0]))
#define MQTTClientMgmt_isClInactive(usr) ((0 == usr->flags) && \
                                          ('\0' != usr->clientID[0]))
...
//
//*****************************************************************************
static inline void setConnectState(MQTTClientMgmt_usr_t *usr,
                                   bool connected)
{
    if(connected)
    {
...
//
//*****************************************************************************
static inline void setCleanSession(MQTTClientMgmt_usr_t *usr,
                                   bool cleanSession)
{
    if(cleanSession)
    {
...
//
//*****************************************************************************
static inline void setAssigned(MQTTClientMgmt_usr_t *usr,
                               bool assignment)
{
    if(assignment)
    {
...
//
//*****************************************************************************
static inline bool qos2PubRxLogup(MQTTClientMgmt_usr_t *usr,
                                  uint16_t msgID)
{
...
//
//*****************************************************************************
static inline bool ack2MsgIdLogup(MQTTClientMgmt_usr_t *usr,
                                  uint16_t msgID)
{
...
//
//*****************************************************************************
static inline bool qos2PubRxUnlog(MQTTClientMgmt_usr_t *usr,
                                  uint16_t msgID)
{
...
//
//*****************************************************************************
static inline bool ack2MsgIdUnlog(MQTTClientMgmt_usr_t *usr,
                                  uint16_t msgID)
{
...
//
//*****************************************************************************
static inline bool qos2PubRxIsDone(MQTTClientMgmt_usr_t *usr,
                                   uint16_t msgID)
{
...
//
//*****************************************************************************
static inline int32_t _pubDispatch(MQTTClientMgmt_usr_t *usr,
                                   MQTT_Packet_t *mqp,
                                   bool dup)
{
...
//
//*****************************************************************************
static void pubDispatch(uint32_t clMap,
                        MQTT_Packet_t *mqp)
{
    ...
    for(i = 0; i < MAX_CLIENTS; i++)
    {
        if(clMap & (1 << i))
        {
            ...
            if(isConnected(usr))
            {
                if((_pubDispatch(usr, mqp,
                                 false) > 0) &&
                   MQTTClientMgmt_needToWaitListPublish(qos, usr))
                {
...
//
//*****************************************************************************
static void wlRemove(MQTT_AckWlist_t *list,
                     MQTT_Packet_t *prev,
                     MQTT_Packet_t *elem)
{
    if(prev)
...
static void sessWlMqpDispatch(MQTTClientMgmt_usr_t *usr)
{
    ...
    for(mqp = MQTTClientMgmt_pWlMqpSess->head; NULL != mqp;
        prev = mqp, mqp = next)
    {
        cpy = NULL;
...
//
//*****************************************************************************
static MQTT_Packet_t * wlMqpUnmapFind(MQTT_AckWlist_t *wl,
                                      MQTTClientMgmt_usr_t *usr,
                                      uint16_t msgID,
                                      MQTT_Packet_t **prev)
{
...
//
//*****************************************************************************
static bool wlRmfreeTry(MQTT_AckWlist_t *wl,
                        MQTT_Packet_t *prev,
                        MQTT_Packet_t *mqp)
{
    if(0 == MQTTClientMgmt_mqpMapGet(mqp))
...
//
//*****************************************************************************
static bool ack1UnmapRmfreeTry(MQTTClientMgmt_usr_t *usr,
                               uint16_t msgID)
{
    MQTT_Packet_t *prev = NULL;
    MQTT_Packet_t *mqp = NULL;
...
//
//*****************************************************************************
static void wlPurge(MQTT_AckWlist_t *wl,
                    MQTTClientMgmt_usr_t *usr)
{
    MQTT_Packet_t *mqp = NULL;
    MQTT_Packet_t *prev = NULL;
...
//
//*****************************************************************************
static bool procPubRelRx(MQTTClientMgmt_usr_t *usr,
                         uint16_t msgID)
{
...
//
//*****************************************************************************
static bool procPubRecRx(MQTTClientMgmt_usr_t *usr,
                         uint16_t msgID)
{
    MQTT_Packet_t *prev = NULL;
    MQTT_Packet_t *mqp = NULL;
...
//
//*****************************************************************************
static MQTTClientMgmt_usr_t * assignClUsr(char *clientID,
                                          uint8_t *vhBuf)
{
    MQTTClientMgmt_usr_t *usr;
    ...
    USR_INFO("Assignment of user %s (detail: index %d and name %s) \n\r",
             usr ? "Succeeded" : "Failed", usr ? (int32_t )usr->index : -1,
             usr ? usr->clientID : "");
...
//
//*****************************************************************************
bool MQTTClientMgmt_qos2PubRxUpdate(void *usrCl,
                                    uint16_t msgID)
{
    MQTTClientMgmt_usr_t *usr = (MQTTClientMgmt_usr_t*) usrCl;
...
//
//*****************************************************************************
void MQTTClientMgmt_pubDispatch(uint32_t clMap,
                                MQTT_Packet_t *mqp)
{
                                  ...
                                  bool retain)
{
    ...
    if((NULL == topic) ||
       ((dataLen > 0) && (NULL == dataBuf)) || (NULL == usrCl))
    {
...
//
//*****************************************************************************
bool MQTTClientMgmt_notifyAck(void *usrCl,
                              uint8_t msgType,
                              uint16_t msgID)
{
    MQTTClientMgmt_usr_t *usr = (MQTTClientMgmt_usr_t*) usrCl;
...
//
//*****************************************************************************
uint16_t MQTTClientMgmt_connectRx(void *ctxCl,
                                  bool cleanSession,
                                  char *clientID,
                                  void *appCl,
                                  void *will,
...
//
//*****************************************************************************
void MQTTClientMgmt_onConnackSend(void *usrCl,
                                  bool cleanSession)
{
    MQTTClientMgmt_usr_t *usr = (MQTTClientMgmt_usr_t*) usrCl;
