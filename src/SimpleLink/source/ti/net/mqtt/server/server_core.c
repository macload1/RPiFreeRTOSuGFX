...
//
//*****************************************************************************
static void stack_add(MQTTServerCore_TopicNode_t *node,
                      uint32_t val1,
                      uint32_t val2)
{
...
//
//*****************************************************************************
static inline void node_retain_set(MQTTServerCore_TopicNode_t *node,
                                   bool retain)
{
    if(retain)
    {
...
//
//*****************************************************************************
static inline void node_qid_set(MQTTServerCore_TopicNode_t *node,
                                uint8_t qid)
{
...
static MQTTServerCore_TopicNode_t * alloc_topic_node(void)
{
    ...
    if(MQTTServerCore_NodesInUse < MAX_TOP_NODES)
    {
        ...
        node =
            (MQTTServerCore_TopicNode_t *)malloc(sizeof(
                                                     MQTTServerCore_TopicNode_t));
...
//
//*****************************************************************************
static bool is_end_of_subtop(const char *topstr,
...
//
//*****************************************************************************
static int32_t subtop_read(const char *topstr,
                           char **subtop_buf,
                           uint16_t buf_length,
                           char const **next_subtop)
{
...
//
//*****************************************************************************
static MQTTServerCore_TopicNode_t * alloc_node_subtop(const char *topstr,
                                                      char const **next_subtop)
{
    uint16_t len = 0;
...
//
//*****************************************************************************
bool is_first_subtop(const char *subtop,
                     const char *topstr,
                     char const **next_subtop)
{
    while(*subtop == *topstr)
...
//
//*****************************************************************************
MQTTServerCore_TopicNode_t * subtop_nhbr_node_find(
    const MQTTServerCore_TopicNode_t *root_nh,
    const char *topstr,
    char const **next_subtop)
{
...
//
//*****************************************************************************
MQTTServerCore_TopicNode_t * nhbr_node_find(
    const MQTTServerCore_TopicNode_t *root_nh,
    const char *subtop)
{
...
//
//*****************************************************************************
static MQTTServerCore_TopicNode_t * lowest_node_find(const char *topstr,
                                                     bool *flag_nh,
                                                     char const **next_subtop)
{
...
MQTTServerCore_TopicNode_t * leaf_node_find(const char *topstr)
{
    ...
    bool flag_nh;
    MQTTServerCore_TopicNode_t *node = lowest_node_find(topstr, &flag_nh,
                                                        &next_subtop);
...
//
//*****************************************************************************
static MQTTServerCore_TopicNode_t * hier_nodes_create(
    const char *topstr,
    MQTTServerCore_TopicNode_t
    **leaf)
{
...
//
//*****************************************************************************
static void install_nhbr_node(MQTTServerCore_TopicNode_t *base,
                              MQTTServerCore_TopicNode_t *node)
{
    while(base->dnNhbr)
    {
...
//
//*****************************************************************************
static void set_up_hier_nodes(MQTTServerCore_TopicNode_t *upHier,
                              MQTTServerCore_TopicNode_t *dnHier)
{
...
//
//*****************************************************************************
static MQTTServerCore_TopicNode_t * node_delete(
    MQTTServerCore_TopicNode_t *node)
{
...
//
//*****************************************************************************
static bool topbuf_add(MQTTServerCore_TopbufDesc_t *buf_desc,
                       const MQTTServerCore_TopicNode_t *node)
{
    const char *next_subtop;
...
//
//*****************************************************************************
static bool topbuf_cpy(MQTTServerCore_TopbufDesc_t *buf_desc,
                       const char *subtop)
{
    const char *next_subtop;
    char *buf = buf_desc->buffer + buf_desc->offset;
...
//
//*****************************************************************************
static bool is_node_SUB_subtop(const MQTTServerCore_TopicNode_t *node,
                               const char *subtop)
{
...
//
//*****************************************************************************
static MQTTServerCore_TopicNode_t * pub_hier_search(
    const char *topSUB,
    const
    MQTTServerCore_TopicNode_t *base,
    MQTTServerCore_TopbufDesc_t
    *mk_pubtop)
{
...
//
//*****************************************************************************
static bool is_node_PUB_subtop(const MQTTServerCore_TopicNode_t *node,
                               const char *subtop,
                               bool endtop)
{
...
//
//*****************************************************************************
static MQTTServerCore_TopicNode_t * SUB_leaf_search(
    const char *topPUB,
    const
    MQTTServerCore_TopicNode_t *base)
{
...
static void try_node_delete(MQTTServerCore_TopicNode_t *node)
{
    while(node)
    {
        if(is_node_retain(node) || is_node_willed(node) ||
           enrolls_plugin(node) ||
...
//
//*****************************************************************************
static void pub_msg_send(const MQTT_UTF8String_t *topic,
...
//
//*****************************************************************************
static MQTTServerCore_TopicNode_t * SUB_node_create(const char *topSUB,
                                                    uint8_t qid,
                                                    void *usrCl)
{
...
//
//*****************************************************************************
static uint8_t proc_pub_leaf(MQTTServerCore_TopicNode_t *leaf,
                             ...
                             void *usrCl)
{
    ...
    if(is_node_retain(leaf))
    {
        ...
        pub_msg_send(topic, leaf->myData, leaf->myDlen,
                     MQTT_MAKE_FH_FLAGS(false, qid,
                                        true), MQTTClientMgmt_bmapGet(usrCl));
    }
...
//
//*****************************************************************************
static uint8_t proc_pub_hier_no_top(MQTTServerCore_TopicNode_t *base,
...
//
//*****************************************************************************
static uint8_t proc_pub_tree_no_top(MQTTServerCore_TopicNode_t *base,
                                    MQTTServerCore_TopbufDesc_t *mk_pubtop,
                                    MQTT_QOS qos,
                                    void *usrCl)
{
...
//
//*****************************************************************************
static uint8_t proc_pub_hier_SUBtop(const char *topSUB,
                                    const MQTTServerCore_TopicNode_t *base,
                                    MQTTServerCore_TopbufDesc_t *mk_pubtop,
                                    MQTT_QOS qos,
                                    void *usrCl)
{
...
//
//*****************************************************************************
static uint8_t proc_pub_tree_SUBtop(const char *topSUB,
                                    MQTTServerCore_TopicNode_t *base,
                                    MQTTServerCore_TopbufDesc_t *mk_pubtop,
                                    MQTT_QOS qos,
                                    void *usrCl)
{
    ...
    while(stack_ref < MQTTServerCore_stackIdx)
    {
        ...
        ack =
            proc_pub_hier_SUBtop((char*) stack->val1, stack->node, mk_pubtop,
                                 qos,
                                 usrCl);
...
//
//*****************************************************************************
static uint8_t proc_sub_ml_wc_hier(const char *grandpa_topSUB,
                                   char *parent_subtop,
                                   MQTTServerCore_TopicNode_t *base,
                                   MQTTServerCore_TopbufDesc_t *mk_pubtop,
                                   MQTT_QOS qos,
...
//
//*****************************************************************************
static uint8_t proc_sub_ml_wc_tree(char *grandpa_topSUB,
                                   char *parent_subtop,
                                   MQTTServerCore_TopicNode_t *base,
                                   MQTT_QOS qos,
                                   void *usrCl)
{
    MQTTServerCore_TopbufDesc_t mk_pubtop =
    { MQTTServerCore_workBuf, WBUF_LEN, 0 /* offset */};
    uint32_t stack_ref = MQTTServerCore_stackIdx;
    uint8_t min = QOS_VALUE(qos);
    MQTTServerCore_NodeStack_t *stack;
    uint8_t ack;
    ...
    while(stack_ref < MQTTServerCore_stackIdx)
    {
        ...
        mk_pubtop.offset = stack->val2;
        ack =
            proc_sub_ml_wc_hier((char*) stack->val1, parent_subtop, stack->node,
                                &mk_pubtop, qos,
                                usrCl);
        if(ack < min)
        {
...
//
//*****************************************************************************
static uint8_t ml_wc_nodes_create(char *parent_topSUB,
                                  uint16_t toplen,
                                  uint8_t qid,
                                  void *usrCl)
{
...
//
//*****************************************************************************
static uint8_t proc_sub_ml_wildcard(char *topSUB,
                                    uint16_t toplen,
                                    MQTT_QOS qos,
                                    void *usrCl)
{
...
//
//*****************************************************************************
static uint8_t proc_sub_sl_or_no_wc(const char *topSUB,
                                    MQTT_QOS qos,
                                    void *usrCl)
{
    MQTTServerCore_TopbufDesc_t mk_pubtop =
    ...
    min = proc_pub_tree_SUBtop(topSUB, MQTTServerCore_rootNode, &mk_pubtop, qos,
                               usrCl);
...
//
//*****************************************************************************
static uint16_t proc_forward_slash(char *buf,
                                   uint16_t len)
{
    uint16_t i;
    uint16_t j;
...
//
//*****************************************************************************
static inline bool is_valid_char_order(char prev,
                                       char curr)
{
    return (((('/' != prev) &&
              ('+' == curr)) || (('+' == prev) && ('/' != curr)) ||
             (('/' != prev) &&
              ('#' == curr)) || (('#' == prev))) ? false : true);
}
...
//
//*****************************************************************************
static bool is_valid_SUB_top(const char *buf,
                             uint16_t len)
{
    char prev;
    char curr;
...
//
//*****************************************************************************
static bool proc_sub_msg_rx(void *usrCl,
                            ...
                            uint8_t *ack)
{
    ...
    for(i = 0; i < nTopics; i++)
    {
        ...
        ack[i] = ('#' == buf[len - 1]) ? proc_sub_ml_wildcard(buf, len, qos,
                                                              usrCl) :
...
//
//*****************************************************************************
static bool proc_sub_msg_rx_locked(void *usrCl,
                                   const MQTT_UTF8StrQOS_t *qosTopics,
                                   uint32_t nTopics,
                                   uint16_t msgID,
                                   uint8_t *ack)
...
//
//*****************************************************************************
static void leaf_un_sub(MQTTServerCore_TopicNode_t *leaf,
                        void *usrCl)
{
    uint8_t j = 0;
...
//
//*****************************************************************************
static bool proc_un_sub_msg(void *usrCl,
                            ...
                            uint16_t msgID)
{
    ...
    for(i = 0; i < nTopics; i++)
    {
        ...
        if('#' == MQTTServerCore_workBuf[len - 1]) /* Multi-level Wildcard */
        {
...
//
//*****************************************************************************
static bool proc_un_sub_msg_locked(void *usrCl,
                                   const MQTT_UTF8String_t *topics,
                                   uint32_t nTopics,
                                   uint16_t msgID)
{
...
//
//*****************************************************************************
static void leaf_msg_send(const MQTTServerCore_TopicNode_t *leaf,
                          const MQTT_UTF8String_t *topic,
                          const uint8_t *dataBuf,
                          uint32_t dataLen,
                          bool dup,
                          ...
                          bool retain)
{
    ...
    {
        MQTTServerPlug_publish(leaf->pgMap, topic, dataBuf, dataLen, dup, qos,
                               retain);
    }
    return;
}
...
//
//*****************************************************************************
static void node_data_set(MQTTServerCore_TopicNode_t *node,
                          uint8_t *data,
                          uint32_t dlen,
                          uint8_t qid,
                          bool retain)
...
//
//*****************************************************************************
static bool node_data_update(MQTTServerCore_TopicNode_t *node,
                             bool drop_qid0,
                             const uint8_t *dataBuf,
                             uint32_t dataLen,
                             uint8_t qid,
...
//
//*****************************************************************************
static int32_t pub_topic_read(const MQTT_UTF8String_t *topic,
                              char *buf,
                              uint32_t len)
{
    uint32_t i = 0;
...
//
//*****************************************************************************
static void proc_sub_tree_topPUB(const char *topPUB,
                                 const MQTT_UTF8String_t *topic,
                                 const uint8_t *dataBuf,
                                 uint32_t dataLen,
                                 MQTT_QOS qos,
...
//
//*****************************************************************************
static bool _proc_pub_msg_rx(void *usrCl,
                             const MQTT_UTF8String_t *topic,
                             const uint8_t *dataBuf,
                             uint32_t dataLen,
                             uint8_t msgID,
                             ...
                             bool retain)
{
       ...
       (proc_forward_slash(MQTTServerCore_workBuf, topic->length) > 0))
    {
        ...
        err = -2;
        if((msgID == 0) || (MQTT_QOS2 != qos) ||
           (true == MQTTClientMgmt_qos2PubRxUpdate(usrCl, msgID)))
        {
            /* Forward data to all subscribers of PUB topic in server */
            proc_sub_tree_topPUB(MQTTServerCore_workBuf, topic, dataBuf,
                                 dataLen, qos,
                                 false);
            ...
            if(retain)
            {
                MQTTServerCore_TopicNode_t *leaf = topic_node_create(
                    MQTTServerCore_workBuf);
                if((NULL == leaf) ||
                   (false ==
                    node_data_update(leaf, false /*true*/, dataBuf, dataLen,
    ...
    DBG_INFO("Processing of PUB message from %s (0x%08x) has %s (%d)\n\r",
             usrCl ? "client" : "plugin", usrCl ? MQTTClientMgmt_bmapGet(
                 usrCl) : 0, err ? "failed" : "succeeded", err);
...
//
//*****************************************************************************
static bool proc_pub_msg_rx_locked(void *usrCl,
                                   const MQTT_UTF8String_t *topic,
                                   const uint8_t *dataBuf,
                                   uint32_t dataLen,
                                   uint16_t msgID,
...
//
//*****************************************************************************
static int32_t utf8_str_rd(const MQTT_UTF8String_t *utf8,
                           char *buf,
                           uint32_t len)
{
...
//
//*****************************************************************************
static bool utf8_str_set(MQTT_UTF8String_t *utf8,
                         const char *buf,
                         const uint32_t len)
{
...
static bool will_message_construct(MQTTServerCore_WillParams_t *willParam,
                                   MQTTServerCore_TopicNode_t *leaf,
                                   MQTT_UTF8String_t *willMsgData,
                                   uint8_t QoS,
                                   bool messageRetain)
{
    bool ret_val = true;
    ...
    ret_val =
        utf8_str_set(&(willParam->willMsg), MQ_CONN_UTF8_BUF(willMsgData),
                     (MQ_CONN_UTF8_LEN(willMsgData)));
...
//
//*****************************************************************************
static uint16_t proc_connect_rx(void *ctxCl,
                                uint8_t connFlags,
                                MQTT_UTF8String_t * const *utf8Vec,
                                void **usrCl)
{
    MQTTServerCore_TopicNode_t *leaf = NULL;
    ...
    void *appCl = NULL;
    uint16_t utf8_len = 0;
    uint16_t rv = MQTTServerPlug_connect(MQC_UTF8_CLIENTID(
                                             utf8Vec),
                                         MQC_UTF8_USERNAME(
                                             utf8Vec),
                                         MQC_UTF8_PASSWORD(utf8Vec), &appCl);
    ...
    if(rv == 0)
    {
        ...
        if(utf8 && utf8->length)
        {
            ...
            if(false ==
               will_message_construct(willParams, leaf,
                                      MQC_UTF8_WILL_MSG(utf8Vec),
                                      CONN_FLAGS_WQID_GET(connFlags),
                                      connFlags &
                                      MQTT_CONN_MSG_FLAG_WILL_RETAIN))
            {
        ...
        rv =
            MQTTClientMgmt_connectRx(
                ctxCl,
                (connFlags & MQTT_CONN_MSG_FLAG_CLEAN_START) ? true : false,
                utf8_len ? MQTTServerCore_workBuf : NULL,
                appCl, willParams, usrCl);
...
//
//*****************************************************************************
static uint16_t proc_connect_rx_locked(void *ctxCl,
                                       uint8_t connFlags,
                                       MQTT_UTF8String_t * const *utf8Vec,
                                       void **usrCl)
{
...
//
//*****************************************************************************
static void session_hier_delete(MQTTServerCore_TopicNode_t *node,
                                void *usrCl)
{
    MQTTServerCore_TopicNode_t *prev = NULL;
...
//
//*****************************************************************************
static void proc_client_will(MQTTServerCore_TopicNode_t *leaf,
                             MQTTServerCore_WillParams_t *willParams)
{
    ...
    /* Forward data to all subscribers of PUB topic in server */
    proc_sub_tree_topPUB((const char*) topic.buffer, &topic,
                         (const uint8_t *) willParams->willMsg.buffer,
                         willParams->willMsg.length, MK_QOS_ENUM(
                             willParams->willQoS), willParams->retain);

    /* If retain parameter of the will message is set to
       true, store the retained message */
    if(willParams->retain)
    {
        node_data_update(leaf, false,
                         (const uint8_t *)willParams->willMsg.buffer,
                         willParams->willMsg.length, willParams->willQoS,
                         willParams->retain);
    }
}
...
//
//*****************************************************************************
static void onClNetClose(void *usrCl,
                         bool due2err)
{
    MQTTServerCore_TopicNode_t *leaf = NULL;
    MQTTServerCore_WillParams_t *willParams = NULL;
    ...
    willParams = (MQTTServerCore_WillParams_t*) MQTTClientMgmt_willHndlGet(
        usrCl);
    if(NULL != willParams)
    {
...
//
//*****************************************************************************
static void on_cl_net_close_locked(void *usrCl,
                                   bool due2err)
{
...
//
//*****************************************************************************
static void onConnackSend(void *usrCl,
                          bool cleanSession)
{
...
//
//*****************************************************************************
static void on_connack_send_locked(void *usrCl,
                                   bool cleanSession)
{
...
//
//*****************************************************************************
static bool proc_notify_ack_locked(void *usrCl,
                                   uint8_t msgType,
                                   uint16_t msgID)
{
...
//
//*****************************************************************************
static int32_t proc_topic_enroll(uint8_t pgID,
                                 const MQTT_UTF8String_t *topic,
                                 MQTT_QOS qos)
{
    MQTTServerCore_TopicNode_t *leaf = NULL;
...
//
//*****************************************************************************
static int32_t proc_topic_enroll_locked(uint8_t pgID,
                                        const MQTT_UTF8String_t *topic,
                                        MQTT_QOS qos)
{
    int32_t rv = 0;
...
//
//*****************************************************************************
static int32_t proc_topic_cancel(uint8_t pgID,
                                 const MQTT_UTF8String_t *topic)
{
    MQTTServerCore_TopicNode_t *leaf = NULL;
    uint16_t len = 0;
...
//
//*****************************************************************************
static int32_t proc_topic_cancel_locked(uint8_t pgID,
                                        const MQTT_UTF8String_t *topic)
{
    int32_t rv = 0;
...
//
//*****************************************************************************
static int32_t proc_app_pub_send_locked(const MQTT_UTF8String_t *topic,
                                        const uint8_t *dataBuf,
                                        uint32_t dataLen,
                                        MQTT_QOS qos,
                                        bool retain)
                            ...
                            const MQTTServerCore_AppCfg_t *appCfg)
{
    ...
    MQTTServerUtil_setParams(libCfg->debugPrintf, libCfg->mutex,
                             libCfg->mutexLockin,
                             libCfg->mutexUnlock);
