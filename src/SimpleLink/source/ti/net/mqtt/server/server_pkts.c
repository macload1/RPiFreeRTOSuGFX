...
#define USR_INFO debugPrintf
#define DBG_INFO(I, ...) if(MQTTServerPkts_auxDbgEn) debugPrintf(I, \
                                                                 ## __VA_ARGS__)
...
//globals
//*****************************************************************************
static void *MQTTServerPkts_mutex = NULL;
static void (*mutexLockin)(void*) = NULL;
static void (*mutexUnlock)(void*) = NULL;
static bool MQTTServerPkts_auxDbgEn = false;
static int32_t (*debugPrintf)(const char *fmt,
                              ...) = NULL;
...
//
//*****************************************************************************
static int32_t mqp_buf_rd_utf8(const uint8_t *buf,
                               const uint8_t *end,
...
static int32_t loopb_trigger(void)
{
    ...
    {
        rv =
            MQTTServerPkts_netOps->sendTo(MQTTServerPkts_loopbNet,
                                          MQTTServerPkts_loopData,
                                          LOOP_DLEN,
                                          MQTTServerPkts_loopbackPort, ip_addr,
                                          4);
        if(0 == rv)
        {
...
//
//*****************************************************************************
static void do_net_close_rx(MQTT_ClientCtx_t *clCtx,
                            bool due2err)
{
...
//
//*****************************************************************************
static void do_net_close_tx(MQTT_ClientCtx_t *clCtx,
                            bool due2err)
{
    if(due2err)
    {
...
//
//*****************************************************************************
static int32_t cl_ctx_send(MQTT_ClientCtx_t *clCtx,
                           uint8_t *buf,
                           uint32_t len)
{
    ...
    USR_INFO("S: FH-B1 0x%02x, len %u to net %d: %s [@ %u]\n\r", *buf, len,
             clCtx->net, rv ? "Sent" : "Fail",
             MQTTServerPkts_netOps->time());
    return(rv);
}
...
//
//*****************************************************************************
static int32_t vh_msg_send(MQTT_ClientCtx_t *clCtx,
                           uint8_t msgType,
                           MQTT_QOS qos,
                           bool hasVH,
                           uint16_t vhData)
...
//
//*****************************************************************************
static int32_t _mqtt_vh_msg_send(void *ctxCl,
                                 uint8_t msgType,
                                 MQTT_QOS qos,
                                 bool hasVH,
                                 uint16_t vhData)
...
//
//*****************************************************************************
int32_t MQTTServerPkts_sendVhMsg(void *ctxCl,
                                 uint8_t msgType,
                                 MQTT_QOS qos,
                                 bool hasVH,
                                 uint16_t vhData)
...
//
//*****************************************************************************
int32_t MQTTServerPkts_sendLockedVhMsg(void *ctxCl,
                                       uint8_t msgType,
                                       MQTT_QOS qos,
                                       bool hasVH,
                                       uint16_t vhData)
...
//
//*****************************************************************************
int32_t mqtt_connack_send(void *ctxCl,
                          uint8_t *vhBuf)
{
    MQTT_ClientCtx_t *clCtx = (MQTT_ClientCtx_t *) ctxCl;
...
//
//*****************************************************************************
static int32_t _mqtt_server_pub_dispatch(void *ctxCl,
                                         MQTT_Packet_t *mqp,
                                         bool dup)
{
    int32_t rv = 0;
    ...
    rv =
        cl_ctx_send((MQTT_ClientCtx_t*) ctxCl, buf, MQTT_PACKET_CONTENT_LEN(mqp));
...
//
//*****************************************************************************
int32_t MQTTServerPkts_dispatchPub(void *ctxCl,
                                   MQTT_Packet_t *mqp,
                                   bool dup)
{
...
//
//*****************************************************************************
int32_t MQTTServerPkts_dispatchLockedPub(void *ctxCl,
                                         MQTT_Packet_t *mqp,
                                         bool dup)
{
    int32_t rv;
...
//
//*****************************************************************************
static int32_t sub_ack_send(MQTT_ClientCtx_t *clCtx,
                            uint8_t *buf,
                            uint8_t pay_ofs,
                            uint32_t payLen,
                            uint16_t msgID)
{
    ...
    ref -= MQTT_packetBufTailWrRemlen(ref - MQTT_MAX_REMLEN_BYTES,
                                      payLen + buf - ref);

    ref -= 1;
    *ref =
        MQTT_MAKE_FH_BYTE1(MQTT_SUBACK,
...
//
//*****************************************************************************
static inline int32_t unsub_ack_send(MQTT_ClientCtx_t *clCtx,
                                     uint16_t msgID)
{
...
//
//*****************************************************************************
static int32_t buf_rd_utf8_qos(uint8_t *buf,
                               uint8_t *end,
                               MQTT_UTF8StrQOS_t *utf8_qos)
{
    MQTT_UTF8String_t utf8;
    uint8_t *ref = buf;
...
//
//*****************************************************************************
static bool _proc_sub_msg_rx(MQTT_Packet_t *mqpRaw,
                             MQTT_UTF8StrQOS_t *qosTopics,
                             uint32_t *nTopics)
{
    uint8_t *buf, *end;
    uint32_t i = 0;
...
//
//*****************************************************************************
static bool proc_sub_msg_rx(MQTT_ClientCtx_t *clCtx,
                            MQTT_Packet_t *mqpRaw)
{
    ...
    if(MQTTServerPkts_usrCBs->subMsgRx(clCtx->usr, qosTopics, nTopics, msgID,
                                       ack + MQP_SUBACK_PAY_OFS))
    {
...
//
//*****************************************************************************
static bool _proc_unsub_msg_rx(MQTT_Packet_t *mqpRaw,
                               MQTT_UTF8String_t *topics,
                               uint32_t *nTopics)
{
    uint8_t *buf, *end;
    uint32_t i = 0;
...
//
//*****************************************************************************
static bool proc_unsub_msg_rx(MQTT_ClientCtx_t *clCtx,
                              MQTT_Packet_t *mqpRaw)
{
    uint32_t nTopics = MQP_MAX_TOPICS;
    uint16_t msgID;
...
//
//*****************************************************************************
static bool proc_pub_msg_rx(MQTT_ClientCtx_t *clCtx,
                            MQTT_Packet_t *mqpRaw)
{
    ...
    rv =
        MQTTServerPkts_usrCBs->pubMsgRx(clCtx->usr, &topic,
                                        MQTT_PACKET_PUB_PAY_BUF(
                                            mqpRaw),
                                        MQTT_PACKET_PUB_PAY_LEN(
                                            mqpRaw), msgID,
                                        MQTT_FH_BYTE1_IS_DUP(
                                            B), qos, MQTT_FH_BYTE1_IS_RETAIN(B));
    if(false == rv)
    {
        return(rv);
    }
...
//
//*****************************************************************************
static bool proc_ack_msg_rx(MQTT_ClientCtx_t *clCtx,
                            MQTT_Packet_t *mqpRaw)
{
    if((false == MQTT_packetProcMsgIdAckRx(mqpRaw, false)) ||
       (false ==
        MQTTServerPkts_usrCBs->ackNotify(clCtx->usr, mqpRaw->msgType,
                                         mqpRaw->msgID)))
    {
        return(false);
    }
...
//
//*****************************************************************************
static bool proc_protocol_info(MQTT_UTF8String_t *utf8,
                               uint8_t ver)
{
    const char *buf = utf8->buffer;

    /* Check for protocol version 3.1.1 */
    if((4 == utf8->length) && (buf[0] == 'M') && (buf[1] == 'Q') &&
       (buf[2] == 'T') &&
       (buf[3] == 'T') && (0x04 == ver))
    {
        return(true);
    ...
    if((6 == utf8->length) && (buf[0] == 'M') && (buf[1] == 'Q') &&
       (buf[2] == 'I') &&
...
//
//*****************************************************************************
static uint16_t proc_connect_vh_rx(MQTT_Packet_t *mqpRaw,
                                   uint8_t *connFlags,
                                   uint16_t *kaSecs)
{
    MQTT_UTF8String_t utf8;
...
//
//*****************************************************************************
uint16_t proc_connect_pl_rx(const uint8_t *buf,
                            const uint8_t *end,
                            uint8_t connFlags,
                            MQTT_UTF8String_t *free_utf8s,
                            MQTT_UTF8String_t **used_refs)
{
    MQTT_UTF8String_t *utf8;
    ...
    if((0 == (connFlags & MQTT_CONN_MSG_FLAG_USER_NAME)) &&
       (0 != (connFlags & MQTT_CONN_MSG_FLAG_PASS_WORD)))
    {
...
//
//*****************************************************************************
static bool proc_connect_rx(MQTT_ClientCtx_t *clCtx,
                            MQTT_Packet_t *mqpRaw)
{
    ...
    if(ack_vh16 == 0)
    {
        ...
        *end = 0;
        ...
        ack_vh16 = proc_connect_pl_rx(buf, end, connFlags, free_utf8s,
                                      used_refs);
        if(ack_vh16 == 0)
        {
            cleanSession =
            ...
            if(ack_vh16 == 0)
            {
                ...
                ack_vh16 =
                    MQTTServerPkts_usrCBs->connectRx(clCtx, connFlags,
                                                     &used_refs[0],
                                                     &clCtx->usr);
            }
        }
...
//
//*****************************************************************************
static uint32_t recv_hvec_load(int32_t *recv_hvec,
                               uint32_t size,
...
//
//*****************************************************************************
static bool process_recv(MQTT_ClientCtx_t *clCtx,
                         MQTT_Packet_t *mqpRaw)
{
    ...
    USR_INFO("S: Rcvd msg Fix-Hdr (Byte1) 0x%02x from net %d [@ %u]\n\r",
             mqpRaw->fhByte1, clCtx->net,
             MQTTServerPkts_netOps->time());

    if((MQTT_CONNECT != msgType) ^ had_rcvd_conn_msg(clCtx))
    {
        DBG_INFO("S: Processing of MSG ID 0x%02x: Fail\n\r", mqpRaw->msgID);
        return(rv);
    }
    /* Proto Violation */
...
static void ka_sequence(uint32_t *secs2wait)
{
    ...
    if(((NULL != clCtx) &&
        (MQTT_KA_TIMEOUT_NONE == clCtx->timeout)) || ((NULL == clCtx)))
    {
...
//
//*****************************************************************************
static int32_t accept_ctx(int32_t net,
                          uint32_t waitSecs)
{
    ...
    if(NULL == clCtx)
    {
        USR_INFO("S: No free clCtx, max # of clients reached \n\r");
        return(true);
    }

    clCtx->net = MQTTServerPkts_netOps->accept(net, clCtx->remoteIP,
                                               &clCtx->ipLen);
    if(0 > clCtx->net)
    {
...
//
//*****************************************************************************
static inline int32_t net_recv(int32_t net,
                               MQTT_Packet_t *mqp,
                               uint32_t waitSecs,
                               bool *timedOut)
{
    int32_t rv =
        MQTT_PacketRecv(net, MQTTServerPkts_netOps, mqp, waitSecs, timedOut,
                        NULL);
...
static int32_t proc_loopback_recv(int32_t net)
{
    ...
    int32_t rv =
        MQTTServerPkts_netOps->recvFrom(net, buf, LOOP_DLEN, NULL, NULL,
                                        0);
...
//
//*****************************************************************************
static int32_t proc_recv_hnds_locked(int32_t *recv_hnds,
                                     int32_t n_hnds,
                                     uint32_t waitSecs)
{
    int32_t rv = 0;
...
static int32_t net_hnds_mon_loop(uint32_t waitSecs)
{
    ...
    do
    {
        ...
        n_hnds = (int32_t) recv_hvec_load(r_hvec, MAX_NWCONN,
                                          MQTTServerPkts_usedCtxs);
        ...
        {
            n_hnds =
                MQTTServerPkts_netOps->ioMon(MQTTServerPkts_recvHvec,
                                             &send_hvec,
                                             &rsvd_hvec,
                                             secs2wait);
        }
...
int32_t MQTTServerPkts_run(uint32_t waitSecs)   // TBD break into two functions
{
    ...
    if(MQTTServerPkts_loopbackPort)
    {
        MQTTServerPkts_loopbNet = MQTTServerPkts_netOps->open(
            MQTT_DEV_NETCONN_OPT_UDP, NULL, MQTTServerPkts_loopbackPort, NULL);
    ...
    MQTTServerPkts_listenNet = MQTTServerPkts_netOps->listen(
        nwconnInfo, MQTTServerPkts_listenerPort, &MQTTServerPkts_srvSecure);
...
{
    if(net && net->ioMon && net->close && net->send && net->recv &&
       net->time && net->listen)
    {
...
//
//*****************************************************************************
int32_t MQTTServerPkts_libInit(const MQTTServerPkts_LibCfg_t *libCfg,
                               const MQTTServerPkts_MsgCBs_t *msg_cbs)
{
