...
char template_DropboxV2Metadata [] =
    "{\"entries\":[{\"path_display\": string,\"size\": int32}]}";
                                ...
                                uint8_t *pVendorToken)
{
    ...
    strcat((char *)ReqDirCmdBuf,
           "\r\nContent-Type: Application/Json\r\nContent-Length:  ");
    snprintf((char *)&ReqDirCmdBuf[strlen(
                                       (const char *)ReqDirCmdBuf)],
             CONTENT_LENGTH_MAX_STR_LEN, "%d",
    ...
    return(HttpClient_SendReq (SockId, pSendBuf, (uint8_t *)"POST ",
                               pServerName, (uint8_t *)OTA_SERVER_REST_REQ_DIR,
                               "" /*pVendorDir*/,
                               (uint8_t *)OTA_SERVER_REST_HDR, ReqDirCmdBuf /*pVendorToken*/));
}
                                  ...
                                  OtaDirData_t *pOtaDirData)
{
    ...
    {
        _SlOtaLibTrace((
                           "CdnDropbox_ParseRespDir: ERROR HttpClient_RecvSkipHdr, status=%ld\r\n",
                           RespLen));
        return(RespLen);
    }
    ...
    while((pEndBuf - iterator) >= CRLF_STR_LEN)
    {
        ...
        if(strncmp ((const char *)iterator, "\r\n", CRLF_STR_LEN) == 0)
        {
            ...
            jsonLen = (int)strtol((const char *)pRespBuf, (char **)&iterator,
                                  HEXADECIMAL_BASE);
            break;
        }
        else
    ...
    retVal =
        OtaJson_init(template_DropboxV2Metadata,
                                    ...
                                    uint8_t *pVendorToken)
{
    ...
    return(HttpClient_SendReq(SockId, pSendBuf, (uint8_t *)"POST ", pServerName,
                              (uint8_t *)OTA_SERVER_REST_REQ_FILE_URL, "" /*pFileName*/,
                              (uint8_t *)OTA_SERVER_REST_HDR, ReqDirCmdBuf /*pVendorToken*/));
}
                                      ...
                                      uint32_t FileUrlBufSize)
{
    ...
    /* Initialize the JSON module */
    retVal =
        OtaJson_init(template_DropboxV2URL, &(OtaJsonObj.jsonBuffer.fileBuffer),
                     jsonLen);
    if(retVal < 0)
    {
        /* Initializing the JSON module failed, return error */
    ...
    if(retVal < 0)
    {
        ...
        _SlOtaLibTrace((
                           "CdnDropbox_ParseRespDir: ERROR OtaJson_getURL, retVal=%d\r\n",
                           retVal));
        return(retVal);
    }
