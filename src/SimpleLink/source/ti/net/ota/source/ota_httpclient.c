...
int16_t HttpClient_Connect(uint8_t *ServerName,
                           int32_t IpAddr,
                           int32_t Port,
                           int32_t Secured,
                           int32_t NonBlocking)
{
    ...
    {
        /* get host IP by name */
        Status = sl_NetAppDnsGetHostByName(
            (int8_t *)ServerName, (uint16_t)strlen((const char *)ServerName),
            (unsigned long *)&ServerIp, SL_AF_INET);
        if(0 > Status)
        {
            _SlOtaLibTrace((
                               "HttpClient_Connect: ERROR sl_NetAppDnsGetHostByName, status=%ld\r\n",
                               Status));
    ...
    _SlOtaLibTrace(("HttpClient_Connect: IP_ADDR=%u.%u.%u.%u\r\n",
                    ((char *)&ServerIp)[3] & 0xff, ((char *)&ServerIp)[2] &
                    0xff,
                    ((char *)&ServerIp)[1] & 0xff, ((char *)&ServerIp)[0] &
                    0xff));
    ...
    tTimeout.tv_sec = 2;
    tTimeout.tv_usec = 0;
    Status = sl_SetSockOpt(SockId, SL_SOL_SOCKET, SL_SO_RCVTIMEO, &tTimeout,
                           sizeof(SlTimeval_t));
    if(Status < 0)
    {
        sl_Close(SockId);
        _SlOtaLibTrace((
                           "HttpClient_Connect: ERROR sl_SetSockOpt SL_SO_RCVTIMEO, status=%d\r\n",
                           Status));
    ...
    Status =
        sl_SetSockOpt(SockId, SL_SOL_SOCKET, SL_SO_LINGER, &linger,
                      sizeof(linger));
    if(Status < 0)
    {
        sl_Close(SockId);
        _SlOtaLibTrace((
                           "HttpClient_Connect: ERROR sl_SetSockOpt SL_SO_LINGER, status=%d\r\n",
                           Status));
        return(OTA_HTTP_CLIENT_ERROR_CONNECT_SL_SET_SOC_OPT);
    }
    ...
    Status =
        sl_SetSockOpt(SockId, SL_SOL_SOCKET, SL_SO_SECURE_FILES_CA_FILE_NAME,
                      OTA_SERVER_ROOT_CA_CERT, strlen(
                          OTA_SERVER_ROOT_CA_CERT));
    if(Status < 0)
    {
        sl_Close(SockId);
        _SlOtaLibTrace((
                           "HttpClient_Connect: ERROR sl_SetSockOpt SL_SO_SECURE_FILES_CA_FILE_NAME=%s, status=%d\r\n",
    ...
    Status =
        sl_SetSockOpt(SockId, SL_SOL_SOCKET,
                      ...
                      (const char *)ServerName, strlen(
                          (const char *)ServerName));
    if(Status < 0)
    {
        sl_Close(SockId);
        _SlOtaLibTrace((
                           "HttpClient_Connect: ERROR sl_SetSockOpt SL_SO_SECURE_DOMAIN_NAME_VERIFICATION=%s, status=%d\r\n",
                           ServerName, Status));
        return(OTA_HTTP_CLIENT_ERROR_CONNECT_SL_SET_SOC_OPT);
    }
#endif
    ...
    {
        _SlOtaLibTrace((
                           "HttpClient_Connect: WARNING Socket Connect, status=%ld, Ignored...\r\n",
                           Status));
...
#endif
    if((Status == SL_ERROR_BSD_ESECUNKNOWNROOTCA) ||
       (Status == SL_ERROR_BSD_ESECDATEERROR))
    {
        _SlOtaLibTrace((
                           "HttpClient_Connect: WARNING Socket Connect, status=%ld, Ignored...\r\n",
                           Status));
    ...
    if(Status < 0)
    {
        sl_Close(SockId);
        _SlOtaLibTrace((
                           "HttpClient_Connect: ERROR Socket Connect, status=%ld\r\n",
                           Status));
    ...
    if(NonBlocking)
    {
        sl_SetSockOpt(SockId, SL_SOL_SOCKET, SL_SO_NONBLOCKING, &NonBlocking,
                      sizeof(NonBlocking));
    }
 ...
 */
uint8_t * HttpClient_ParseUrl(uint8_t *pUrlName,
                              uint8_t *pServerNameBuf,
                              int16_t *Status)
{
    uint8_t *pBuf;
    int32_t len;
 ...
 */
int16_t HttpClient_SendReq(int16_t SockId,
                           uint8_t *pHttpReqBuf,
                           uint8_t *pReqMethod,
                           uint8_t *pServerName,
                           uint8_t *pUriPrefix,
                           ...
                           uint8_t *pHdrVal)
{
    ...
    Len =
        sl_Send(SockId, pHttpReqBuf, (int16_t)strlen(
                    (const char *)pHttpReqBuf), 0);
                               ...
                               uint32_t *contentLen)
{
    ...
    {
        _SlOtaLibTrace((
                           "HttpClient_RecvSkipHdr: ERROR HttpClient_Recv, status=%ld\r\n",
                           Len));
        return(Len);
    }
    ...
    while(1)
    {
                ...
                if(contentLen != NULL)
                {
                                ...
                                "Content-Length", CONTENT_LENGTH_STR_LEN) == 0)
                    {
                        ...
                        *contentLen = (int)strtol(
                            ...
                            (char **)&pEndOfContentLen, DECIMAL_BASE);
                    }
                }
            }
        ...
        {
            _SlOtaLibTrace((
                               "HttpClient_RecvSkipHdr: search end of header max retries\r\n"));
            return(OTA_HTTP_CLIENT_ERROR_RESP_HDR_END_NOT_FOUND);
        }
    }
}

int16_t HttpClient_Recv(int16_t SockId,
                        void *pBuf,
                        int16_t BufLen,
                        int16_t Flags,
                        int16_t MaxEagain)
...
int16_t HttpClient_RecvAppend(int16_t SockId,
                              uint8_t *pRecvBuf,
                              int16_t RecvBufSize,
                              int16_t RecvLen,
                              int16_t RecvOffset)
{
    ...
    {
        _SlOtaLibTrace((
                           "HttpClient_RecvAppend: ERROR RecvLen=%d, RecvOffset=%d\r\n",
                           RecvLen,
                           RecvOffset));
    ...
    BytesReceived =
        HttpClient_Recv(SockId, &pRecvBuf[UnprocessedSize], RecvBufSize -
                        UnprocessedSize, 0,
                        MAX_EAGAIN_RETRIES);
    if(0 >= BytesReceived)
    {
        _SlOtaLibTrace((
                           "HttpClient_RecvAppend: ERROR HttpClient_Recv, status=%ld\r\n",
                           BytesReceived));
