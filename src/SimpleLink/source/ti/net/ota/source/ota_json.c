...
#define MAX_METADATA_FILENAME       (120)
...
int16_t OtaJson_getArrayMembersCount(handle objHandle,
                                     const char * pKey)
{
    int16_t retVal;
                     ...
                     uint16_t textLen)
{
    ...
    retVal =
        Json_createTemplate(&OtaJsonObj.templateHandle, templateBuff.fileBuffer,
                            templateSize);
    if(retVal < 0)
    {
...
int16_t OtaJson_getURL(uint8_t *pFileUrl,
                       const char *downloadKey)
{
    uint16_t urlLen = MAX_URL_LENGTH;
    int16_t retVal;

    retVal = Json_getValue(OtaJsonObj.jsonObjHandle, downloadKey, pFileUrl,
                           &urlLen);
    ...
    else
    {
        /* free dynamic memory */
        _SlOtaLibTrace((
                           "[OtaJson_getURL] Error: %d  , Couldn't get the data \r\n",
                           retVal));
        OtaJson_destroy();
    }
...
int16_t OtaJson_getMetadataFileName(uint8_t *pFileName,
                                    const char *FileNameKey)
{
    uint16_t fileNameLen = MAX_METADATA_FILENAME;
    int16_t retVal;

    retVal = Json_getValue(OtaJsonObj.jsonObjHandle, FileNameKey, pFileName,
                           &fileNameLen);
    ...
    if(retVal == JSON_RC__OK)
    {
        ...
        if(fileNameLen == 0)
        {
            ...
            _SlOtaLibTrace((
                               "[OtaJson_getMetadataFileName] fileName Length is zero!"));
    ...
    else
    {
        ...
        OtaJson_destroy();
        _SlOtaLibTrace((
                           "[OtaJson_getMetadataFileName] Error: %d  , Couldn't get the data \r\n",
                           retVal));
    }
...
int16_t OtaJson_getMetadataFileSize(uint32_t *pFileSize,
                                    const char *fileSizeKey)
{
    uint16_t fileSizeLen = MAX_METADATA_FILESIZE;
    int16_t retVal;

    retVal = Json_getValue(OtaJsonObj.jsonObjHandle, fileSizeKey, pFileSize,
                           &fileSizeLen);
